...
const colours = ["#f6c700", "#fc7f00", "#ff2975","#f000ff", "#7b00fb", "#8fce00"]
...
export default class Firework {
    ...
    constructor(p:p5, exploded: boolean, x: number, y: number, width: number, height: number){
        this.p = p;
        ...
        this.firework = this.exploded? 
            ...
            new Particle(this.p, this.p.random(0, width), height, this.p.random(-1,1), this.p.random(initialSpeed+5, initialSpeed), this.colour);
	...
	explode(): void {
		...
		for (let i = 0; i < 50; i++) {
			const velocity = p5.Vector.random2D().mult(this.p.random(-explosionSpeed / 2, explosionSpeed))
			const p = new Particle(this.p, this.firework.pos.x, this.firework.pos.y, velocity.x, velocity.y, this.colour);
