  ...
  Sketch = (p: p5): void => {
    ...
    function drawRays() {
      for (const ray of rays) {
        const [isIntersection, px, py, wallIndex] = ray.intersection(
          p.mouseX,
          p.mouseY,
          walls
        );
    ...
    function createWalls(nwalls: number, npoly: number) {
      ...
      for (let i = 0; i < nwalls; i++) {
        const wall = new Wall({
          p: p,
          x1: p.random(0, width),
          y1: p.random(0, height),
          x2: p.random(0, width),
          y2: p.random(0, height),
        });
        walls.push(wall);
      }
      //RANDOM POLYGONS
      for (let j = 0; j < npoly; j++) {
        ...
        for (let i = 1; i <= polyVert.length; i++) {
          const wall = new Wall({
            ...
            x1: polyVert[i - 1].x,
            y1: polyVert[i - 1].y,
            x2: polyVert[i % polyVert.length].x,
            y2: polyVert[i % polyVert.length].y,
          });
          walls.push(wall);
        }
      }
